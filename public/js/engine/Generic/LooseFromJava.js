...
var IntList = function(){
  var i;
  this.data = []; for(i = 0; i < 128; i++) this.data[i] = 0;
  this.num_fields = 0;
  this.num = 0;
  this.cap = 128;
  this.free_element = -1;
...
IntList.prototype = {
  IntList: function(start_num_fields){
      this.num_fields = start_num_fields;
  },
  size: function(){
      return this.num
  },
  get: function(n, field){
      if(n >= 0 && n < this.num && field >= 0 && field < this.num_fields)
        return this.data[n*this.num_fields + field];
      debugger
  },
  set: function(n, field, val){
    if(n >= 0 && n < this.num && field >= 0 && field < this.num_fields)
      return this.data[n*this.num_fields + field] = val;
    debugger
  },
  clear: function(){
    this.num = 0;
  ...
  insert: function(){
    ...
    {
      var index = this.free_element;
      var pos = index * this.num_fields;
...
var Quadtree = function(width, height, start_max_elements, start_max_depth){
  this.max_elements = start_max_elements;
  this.max_depth = start_max_depth;

  // Insert the root node to the qt.
  this.nodes.insert();
  this.nodes.set(0, node_idx_fc, -1);
  this.nodes.set(0, node_idx_num, 0);

  // Set the extents of the root node.
  this.root_mx = width / 2;
  this.root_my = height / 2;
  this.root_sx = root_mx;
  this.root_sy = root_my;
};
Quadtree.prototype = {
  insert: function(id, x1, y1, x2, y2){
